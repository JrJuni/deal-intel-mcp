...
처음에는 MongoDB 없이 내장 샘플 데이터로 바로 써볼 수 있다. 실제 팀 데이터를
쌓을 준비가 되면 같은 패키지를 MongoDB Atlas 기반 `full` 모드나 유료 인프라용
`pro` 모드로 전환한다. Claude Desktop이나 MCP를 연결한 Codex에서 말로 하면
된다. 별도 CRM 앱은 없다.
...
이 서버는 회의록을 넣으면 LLM이 이 7가지를 자동으로 추출한다. `sample` 모드에서는
내장 가상 데이터로 같은 분석 화면을 먼저 체험하고, `full` 모드에서는 실제 딜 데이터를
MongoDB Atlas에 저장해 패턴 분석까지 한다.
...
사용자 config가 아직 없고 미리보기 결과가 괜찮으면 sample 모드를 저장한다.

```bash
deal-intel config init --profile sample
```

이미 `~/.deal-intel/config.yaml`이 있으면 바로 덮어쓰지 말고 switch를 먼저
미리본다.
...
`config switch ... --force`는 profile-managed key 변경을 의도적으로 승인한 뒤에만
쓴다.
...
이 경로는 내장 가상 데이터만 사용한다. MongoDB, 유료 API, Atlas Vector Search,
DB write가 필요 없다.

**5단계 — 선택: Claude Desktop 연결**

MCP 번들은 [`mcpb/README.md`](mcpb/README.md)에 따라 빌드하거나 설치한다.
번들 입력 폼에서는 `local_sample` storage로 가볍게 시작하거나, 실제 데이터용
`mongo` storage를 연결할 수 있다. Atlas Vector Search까지 쓰는 `pro` 경로는
나중에 `deal-intel config switch pro`로 전환한다.

실제 Atlas-backed 운영을 하려면 `.env.example`을 `.env`로 복사하거나 MCP 번들
입력 폼에 다음 값을 넣는다.
...
MCP 도구 목록이 로드되면 완료다. 현재 서버는 22개 도구를 노출하며,
...
Sample 모드는 의도적으로 read-only다. 실제 create/update/delete 저장이나
semantic `search_deals`는 real storage가 연결된 뒤 사용한다.
...
[Claude Desktop / Codex — 자연어 입력]
         │ stdio JSON-RPC
         ▼
[deal-intel-mcp  FastMCP 서버  22 tools]
         │
         ├── LLM Provider
         ...
         │
         └── Storage
               ├── local_sample  : 내장 read-only 가상 데이터
...
```
src/deal_intel/
  mcp_server.py         FastMCP 진입점, 22개 tool 등록
...
첫 실행 sample 모드에는 필요 없다. 실제 팀 데이터 운영도 기본 기능은 M0 무료
플랜으로 동작한다. `search_deals`도 M0에서는 Python cosine으로 계산한다. 딜 수가
커지면 M10+에서 Atlas Vector Search로 전환할 수 있다.
