...
| `get_insights` | `pipeline.insights` | `tools/get_insights.py`, `schema/pipeline_metrics.py` | legacy query type, filters | legacy insight payloads and pipeline overview aliases | `tests/test_metric_contract.py`, `tests/test_pipeline_metrics_summary.py` | Keep compatibility aliases stable while moving core KPIs to shared calculators. |
...
| `search_deals` | `search.deals` | `tools/search_deals.py`, `providers/embedding.py` | query, filters, limit | semantic/similarity result or unsupported-mode response | `tests/test_search_deals_startup.py` | BI paths must not depend on embeddings. |
