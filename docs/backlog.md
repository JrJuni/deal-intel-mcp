...
3. 다음: `storage.backend: mongo | local_sample` config 추가
4. `LocalSampleClient` 또는 storage adapter 추가
5. MCP startup 시 Mongo env가 없으면 명시적으로 sample mode로 안내
6. zero-config smoke test와 README/quickstart 별도 섹션 추가
