...
| `get_user_memory` | None | `category`, `custom_doc_slug`, `limit` | `ok`, `memory_dir`, `filters`, `documents`, `summary`, `warnings` | Read only; reads safe Markdown files from `~/.deal-intel/user-memory` or configured `user_memory.dir` for assistant context loading. Relative configured paths resolve under `~/.deal-intel`, not the MCP host working directory. Excludes sample templates from broad reads |
| `record_user_memory` | `content` | `category`, `custom_doc_slug`, `title`, `source`, `importance`, `tags` | `ok`, `entry_id`, `memory_dir`, `path`, `category`, `document`, `is_custom_document`, `bytes_written`, `secret_scan` | Appends durable user feedback to safe Markdown files under `~/.deal-intel/user-memory` or configured `user_memory.dir`; relative configured paths resolve under `~/.deal-intel`, not the MCP host working directory. Rejects unsafe paths, non-Markdown custom slugs, and secret-shaped content before writing |
