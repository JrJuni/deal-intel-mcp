...
| `get_customer_theme_evidence` | `theme_key` | `dimension`, `stage`, `industry`, `limit`, `min_importance` | `ok`, `filters`, `summary`, `evidence`, `warnings` | Read only; returns curated customer-theme evidence snippets plus safe source metadata (`interaction_type`, `source_confidence`, `subject`) and excludes raw notes, raw interaction content, contacts, and embeddings |
