...
| `get_deal_review` | `deal_id` | `as_of` | `ok`, `as_of`, `timezone`, `generated_at`, `review` | Read only; uses the restricted metric projection, separates health quality from evidence coverage, suppresses uncalibrated win probability numbers, and excludes raw notes, contacts, and embeddings |
...
wheel build -> passed
FastMCP runtime registration -> 21 tools
