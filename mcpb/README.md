...
- **`storage_backend`** - choose `local_sample` for the zero-config read-only
  demo, or `mongo` for real Atlas-backed data.
...
mcpb pack . deal-intel-mcp-0.1.9.mcpb   # output goes into mcpb/ folder
mcpb info deal-intel-mcp-0.1.9.mcpb
```
...
2. Leave MongoDB/API-key fields blank.
3. Restart Claude Desktop and run `config_doctor(offline=true)`.
4. After the sample path works, switch to `mongo` and provide `mongodb_uri` for
   real data.
